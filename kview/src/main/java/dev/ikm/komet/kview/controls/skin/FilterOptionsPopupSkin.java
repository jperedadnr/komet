...
//                applyButton.setDisable(control.getFilterOptions().equals(filterOptions));
                if (!defaultFilterOptions.equals(filterOptions)) {
                    System.out.println("CURRENT FILTER OPTIONS " + filterOptions);
                    System.out.println("defaultFilterOptions = " + defaultFilterOptions);
                }
    ...
    public FilterOptionsPopupSkin(FilterOptionsPopup control) {
        ...
        ScrollPane scrollPane = new ScrollPane(accordionBox);
    ...
    private void updateCurrentFilterOptions() {
        ...
        accordionBox.updateLangPanes(pane -> {
            ...
            currentFilterOptions.setLangCoordinates(0, languageCoordinates);
        });
        currentFilterOptionsProperty.set(currentFilterOptions);
    }

    private void createAccordionBoxPanes() {
        System.out.println("CREATE ACCORDION BOX PANES");
    ...
    private void setAvailableOptionsFromNavigator(FilterOptions options, Navigator navigator) {
        ...
        List<String> descendentsList = ConceptNavigatorUtils.getDescendentsList(navigator, rootNid, FilterOptions.OPTION_ITEM.LANGUAGE.getPath());
    ...
    private void setInitialOptions(FilterOptions.Option option) {
        ...
        if (option.isMultiSelectionAllowed()) {
            optionForItem.selectedOptions().addAll(option.selectedOptions());
        } else {
            optionForItem.selectedOptions().add(option.selectedOptions().getFirst());
        }
        optionForItem.defaultOptions().addAll(option.selectedOptions());
    }
    ...
    static class AccordionBox extends VBox {
        ...
        public AccordionBox() {
            ...
            TitledPane languageTitledPane = new TitledPane(resources.getString("language.title"), langAccordion);
